...
class VideoPlayer:
    def __init__(self, root):
        ...
        self.vlc_playback_path = None  # Path for VLC (may be transcoded for AV1)
        self.transcode_temp_path = None  # Temp file to clean up
    ...
    def get_video_codec(self, filepath):
        """Detect video codec using ffprobe. Returns codec name or None on failure."""
        try:
    ...
    def transcode_av1_to_h264(self, filepath):
        """Transcode AV1 to H.264 in temp file for VLC (avoids D3D11VA get_buffer failures).
        ...
        safe_base = re.sub(r'[^\w\-_.]', '_', base)[:50]
        temp_path = os.path.join(mp4_folder, f"_vlc_{safe_base}.mp4")
        # Reuse existing transcode if source hasn't changed
        if os.path.exists(temp_path) and os.path.getmtime(temp_path) >= os.path.getmtime(filepath):
            return temp_path
        try:
            self.status_var.set("Transcoding AV1 for playback...")
            self.root.update()
            result = subprocess.run(
                ["ffmpeg", "-y", "-i", filepath, "-c:v", "libx264", "-preset", "fast",
                 "-crf", "23", "-c:a", "aac", "-movflags", "+faststart", temp_path],
    ...
    def load_video(self, filepath):
        """Load a video file."""
        # Recreate VLC Instance+Player to avoid decoder state corruption when switching videos
        ...
        # Clean up previous transcode temp file (only if loading a different video)
        if self.transcode_temp_path and self.video_path != filepath and os.path.exists(self.transcode_temp_path):
            try:
                os.remove(self.transcode_temp_path)
            except OSError:
                pass
        if self.video_path != filepath:
            self.transcode_temp_path = None
            self.vlc_playback_path = None
        ...
        # For AV1: transcode to H.264 for VLC (avoids D3D11VA get_buffer failures)
        codec = self.get_video_codec(filepath)
        if codec == "av1":
            transcoded = self.transcode_av1_to_h264(filepath)
            if transcoded:
                self.vlc_playback_path = transcoded
                self.transcode_temp_path = transcoded
            else:
                messagebox.showwarning("AV1 Transcode Failed",
                    "Could not transcode AV1 for playback. ffmpeg may be missing. Trying original file.")
                self.vlc_playback_path = filepath
        else:
            self.vlc_playback_path = filepath
        
        # Setup OpenCV for frame extraction (always use original file)
        ...
        # Setup VLC for playback (use transcoded path for AV1)
        playback_path = self.vlc_playback_path or self.video_path
        media = self.vlc_instance.media_new(playback_path)
        ...
        status_msg = f"Loaded: {video_name} | Duration: {self.ms_to_time(self.video_duration)} | FPS: {self.fps:.2f}"
        if self.transcode_temp_path:
            status_msg += " [AV1→H.264 for playback]"
    ...
    def play_video(self):
        ...
        if not self.vlc_player.is_playing():
            ...
            if at_end:
                self.vlc_player.stop()
                media = self.vlc_instance.media_new(self.vlc_playback_path or self.video_path)
                self.vlc_player.set_media(media)
    ...
    def on_closing(self):
        ...
        if self.transcode_temp_path and os.path.exists(self.transcode_temp_path):
            try:
                os.remove(self.transcode_temp_path)
            except OSError:
                pass
