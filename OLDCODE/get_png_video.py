...
class VideoPlayer:
    def __init__(self, root):
        ...
        # Load Text / Create Video
        self.loaded_text_path = None
        self.loaded_text_data = []  # list of (start_sec, end_sec, lines) where lines = list of text lines
    ...
    def setup_styles(self):
        ...
        # Configure styles (reduced font sizes to fit GUI)
        self.style.configure("TFrame", background=self.bg_color)
        self.style.configure("TLabel", background=self.bg_color, foreground=self.fg_color, font=("Segoe UI", 8))
        self.style.configure("TButton", background=self.button_color, foreground=self.fg_color, 
                           font=("Segoe UI", 8, "bold"), padding=6)
    ...
    def setup_gui(self):
        ...
        title_label = tk.Label(self.main_frame, text="🎬 Video Player - Frame Extractor", 
                              font=("Segoe UI", 14, "bold"), bg=self.bg_color, fg=self.accent_color)
        ...
        # Current time label
        self.current_time_label = tk.Label(time_frame, text="00:00:00", font=("Consolas", 10, "bold"),
        ...
        # Total time label
        self.total_time_label = tk.Label(time_frame, text="00:00:00", font=("Consolas", 10, "bold"),
                                        bg=self.bg_color, fg=self.fg_color)
        ...
        tk.Label(time_input_frame, text="Go to time:", font=("Segoe UI", 8),
        ...
        self.time_entry = tk.Entry(time_input_frame, textvariable=self.time_entry_var, 
                                  font=("Consolas", 10), width=10, bg=self.button_color, 
        ...
        # Separator
        tk.Label(time_input_frame, text="|", font=("Segoe UI", 8),
                bg=self.bg_color, fg="#585b70").pack(side=tk.LEFT, padx=10)
        
        # Auto extract duration input
        tk.Label(time_input_frame, text="Auto extract duration:", font=("Segoe UI", 8),
        ...
        tk.Label(time_input_frame, text="Every:", font=("Segoe UI", 8),
                bg=self.bg_color, fg=self.fg_color).pack(side=tk.LEFT, padx=(10, 5))
        ...
        # Speed label
        self.speed_label = tk.Label(time_input_frame, text="Speed: 1.0x", font=("Segoe UI", 8),
        ...
        tk.Label(speed_container, text="Playback Speed:", font=("Segoe UI", 8),
        ...
        self.custom_speed_entry = tk.Entry(speed_container, textvariable=self.custom_speed_var,
                                          font=("Consolas", 9), width=6, bg=self.button_color,
                                          fg=self.fg_color, insertbackground=self.fg_color,
        ...
        tk.Label(speed_container, text="|", font=("Segoe UI", 8),
        ...
        self.start_time_entry = tk.Entry(extract_container, textvariable=self.start_time_var,
                                         font=("Consolas", 9), width=10, bg=self.button_color,
                                         fg=self.fg_color, insertbackground=self.fg_color,
                                         relief=tk.FLAT, bd=5)
        ...
        # Load Text - right of Get Video Time
        self.load_text_btn = ttk.Button(extract_container, text="📄 Load Text", 
                                       command=self.load_text_file, style="TButton")
        self.load_text_btn.pack(side=tk.LEFT, padx=10)
        
        # Font Size - right of Load Text
        tk.Label(extract_container, text="Font Size:", font=("Segoe UI", 8),
                bg=self.bg_color, fg=self.fg_color).pack(side=tk.LEFT, padx=(0, 2))
        self.font_size_var = tk.StringVar(value="35")
        self.font_size_entry = tk.Entry(extract_container, textvariable=self.font_size_var,
                                        font=("Consolas", 9), width=4, bg=self.button_color,
                                        fg=self.fg_color, insertbackground=self.fg_color,
                                        relief=tk.FLAT, bd=3)
        self.font_size_entry.pack(side=tk.LEFT, padx=(0, 2))
        
        # Create Video - right of Font Size
        self.create_video_btn = ttk.Button(extract_container, text="🎥 Create Video", 
                                           command=self.create_video, style="Extract.TButton")
        self.create_video_btn.pack(side=tk.LEFT, padx=10)
        ...
        self.status_bar = tk.Label(self.main_frame, textvariable=self.status_var, 
                                  font=("Segoe UI", 8), bg="#11111b", fg=self.fg_color,
    ...
    def load_video(self, filepath):
        ...
        # Setup VLC for playback (parse duration when OpenCV reports no frames — common for MKV)
    ...
    def load_text_file(self):
        """Load a text file (English or Chinese) with format: time, text, time, text, ...
        Comma in text separates lines on the same page."""
        ...
        try:
            ...
            messagebox.showerror("Error", f"Failed to read text file:\n{e}")
            return
        # Parse: alternate time, text, time, text, ...
        pairs = []
        i = 0
        while i < len(lines):
            line = lines[i].strip()
            if not line:
                i += 1
                continue
            # Try parse as time (number)
            try:
                t = float(line)
                text = ""
                if i + 1 < len(lines):
                    next_line = lines[i + 1].strip()
                    if next_line and not self._is_number(next_line):
                        text = next_line
                        i += 1
                pairs.append((t, text))
            except ValueError:
                # Not a number - skip or treat as text for previous (shouldn't happen in valid format)
                i += 1
                continue
            i += 1
        if not pairs:
            messagebox.showwarning("Invalid Format", "No valid time/text pairs found. Format: time, text, time, text, ...")
            return
        # Build segments: (start_sec, end_sec, lines) where lines = text split by comma
        self.loaded_text_data = []
        for j, (t, text) in enumerate(pairs):
            start_sec = t
            end_sec = pairs[j + 1][0] if j + 1 < len(pairs) else start_sec + 999999
            text_lines = [s.strip() for s in text.split(',') if s.strip()] if text else []
            self.loaded_text_data.append((start_sec, end_sec, text_lines))
        self.loaded_text_path = filepath
        self.status_var.set(f"Loaded: {os.path.basename(filepath)} ({len(self.loaded_text_data)} segments)")
    ...
    def create_video(self):
        """Create a new video with text overlay from loaded text file and input video."""
        if self.video_path is None:
            messagebox.showwarning("Warning", "Please open a video file first.")
            return
        if not self.loaded_text_data:
            messagebox.showwarning("Warning", "Please load a text file first (Load Text).")
            return
        try:
            ...
            messagebox.showwarning("Invalid Font Size", "Please enter a valid font size (8-200).")
            return
        # Output: text file folder, name = video_basename + "_new.mp4"
        text_dir = os.path.dirname(self.loaded_text_path)
        input_basename = os.path.splitext(os.path.basename(self.video_path))[0]
        output_path = os.path.join(text_dir, f"{input_basename}_new.mp4")
        
        self.status_var.set("Creating video with text overlay...")
        self.root.update()
        
        try:
            self._create_video_with_text(
                self.video_path,
                output_path,
                self.loaded_text_data,
                font_size
            ...
            self.status_var.set("Create video failed")
            messagebox.showerror("Error", f"Failed to create video:\n{e}")
    
    def _get_text_for_time(self, sec, segments):
        """Return list of text lines for given time (seconds)."""
        for start_sec, end_sec, lines in segments:
            if start_sec <= sec < end_sec:
                return lines
        return []
    
    def _create_video_with_text(self, input_path, output_path, segments, font_size):
        """Create blank video with text overlay. Duration = final_time - initial_time + 1 sec.
        Extracts audio from input as MP3 and merges audio segment into output."""
        import tempfile
        ...
        # Duration from text: final_time - initial_time + 1 second
        initial_time = min(s[0] for s in segments)
        final_time = max(s[0] for s in segments)  # last time in text file
        duration_sec = final_time - initial_time + 1
        total_frames = int(duration_sec * fps)
        
        # Find font (Chinese + English)
        font_paths = [
        ...
        # Extract audio from input as MP3 (save to text file folder)
        text_dir = os.path.dirname(self.loaded_text_path)
        input_basename = os.path.splitext(os.path.basename(input_path))[0]
        mp3_path = os.path.join(text_dir, f"{input_basename}_audio.mp3")
        self.status_var.set("Extracting audio as MP3...")
        self.root.update()
        subprocess.run(
            ["ffmpeg", "-y", "-i", input_path, "-vn", "-acodec", "libmp3lame", "-q:a", "2", mp3_path],
            capture_output=True, text=True,
            creationflags=subprocess.CREATE_NO_WINDOW if os.name == "nt" else 0
        )
        
        # Extract audio segment for output (initial_time to initial_time + duration_sec)
        fd_audio, temp_audio = tempfile.mkstemp(suffix='.m4a')
        os.close(fd_audio)
        try:
            subprocess.run(
                [
            ...
            # Create blank video (black frames) with text overlay
            ...
            try:
                ...
                for frame_idx in range(total_frames):
                    sec = initial_time + frame_idx / fps
                    lines = self._get_text_for_time(sec, segments)
                    if lines:
                        frame = self._draw_text_on_frame(blank.copy(), lines, font, font_size, width, height)
                    else:
                        frame = blank.copy()
                    out.write(frame)
                    if frame_idx % 30 == 0:
                        self.status_var.set(f"Creating video... {frame_idx}/{total_frames} frames")
                        self.root.update()
                out.release()
                
                # Merge blank video with extracted audio
                ...
                if result.returncode != 0:
                    subprocess.run(
                        ["ffmpeg", "-y", "-i", temp_video, "-c:v", "copy", output_path],
                        capture_output=True, text=True,
                        creationflags=subprocess.CREATE_NO_WINDOW if os.name == "nt" else 0
                    )
                # Remove extracted MP3 after video creation is complete
    ...
    def _draw_text_on_frame(self, frame, lines, font, font_size, width, height):
        """Draw text lines on frame. Returns BGR frame.
        Line spacing: 1/10 of character height between lines."""
        img_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        pil_img = Image.fromarray(img_rgb)
        draw = ImageDraw.Draw(pil_img)
        
        # Use Chinese char for height (Latin "A" underestimates in CJK fonts)
        sample = "字" if lines else "A"
        char_height = font_size
        if hasattr(font, 'getsize'):
            try:
                char_height = font.getsize(sample)[1]
            except Exception:
                pass
            ...
            try:
                ...
                pass
        line_spacing = max(1, int(char_height / 10))
    ...
    def extract_video(self):
        ...
        # When -ss is before -i, ffmpeg resets timestamps to 0, so -to would be interpreted
        # as output duration (wrong). Use -t (duration) = end - start instead.
        duration_ms = end_ms - start_ms
        duration_ffmpeg = self.time_hires_to_ffmpeg(self.ms_to_time_hires(duration_ms))
        if duration_ffmpeg is None:
            messagebox.showerror("Error", "Could not compute duration for ffmpeg.")
            return
            
        # Use ffmpeg: -ss before -i for fast seeking (input seeking), -t for duration
    ...
    def _read_frame_at_ms(self, current_ms):
        ...
        When total_frames is 0, frame-based math used to yield -1 and break reads.
        Millisecond seek is a reliable fallback for MKV/MP4 when POS_FRAMES fails.
        """
    ...
    def _imwrite_png(self, path, frame):
        """Save PNG; cv2.imwrite often fails on Windows with non-ASCII paths."""
        try:
    ...
    def _extract_single_png_ffmpeg(self, current_ms, png_path):
        """Grab one frame at current_ms via ffmpeg (reliable when OpenCV/VLC snapshot fail)."""
    ...
    def _vlc_snapshot_to_path(self, png_path):
        """Save current VLC video output as PNG (matches what you see on screen)."""
        fp = os.path.abspath(png_path)
        try:
            vlc_path = fp.replace("\\", "/") if os.name == "nt" else fp
            rc = self.vlc_player.video_take_snapshot(0, vlc_path, 0, 0)
            if rc != 0:
                return False
            return os.path.isfile(fp) and os.path.getsize(fp) > 0
        except Exception:
    ...
    def _write_current_frame_png(self, current_ms, png_path):
        """
        Try ffmpeg first (separate decoder; works when OpenCV cannot read the same file as VLC),
        then VLC snapshot, then OpenCV.
        """
        if self._extract_single_png_ffmpeg(current_ms, png_path):
            return True, None
        err_ffmpeg = "ffmpeg did not produce a file (missing ffmpeg, bad path, or seek failed)."

        self.vlc_player.set_time(int(current_ms))
        self.root.update()
        self.root.update_idletasks()
        if self._vlc_snapshot_to_path(png_path):
            return True, None
    ...
    def extract_frame(self):
        ...
        # VLC often returns -1 for get_time() until playback; use slider as fallback
        ...
        ok, hint = self._write_current_frame_png(current_ms, png_path)
        if not ok:
            self.png_counter -= 1
            messagebox.showerror(
                "Error",
                "Could not save PNG after trying ffmpeg, VLC snapshot, and OpenCV.\n\n"
                f"Path: {png_path}\n\n"
                "Install ffmpeg and add it to your system PATH if it is missing.\n"
                f"{hint or ''}",
            )
            return
    ...
    def auto_extract_frames(self):
        ...
        while current_ms <= end_ms:
            ...
            saved = self._extract_single_png_ffmpeg(current_ms, png_path)
            if not saved:
                ret, frame = self._read_frame_at_ms(current_ms)
                saved = ret and self._imwrite_png(png_path, frame)
