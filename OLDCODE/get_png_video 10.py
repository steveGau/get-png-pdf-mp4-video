...
class VideoPlayer:
    ...
    def setup_gui(self):
        ...
        # Loop checkbox - right of Play
        self.loop_var = tk.BooleanVar(value=False)
        loop_frame = tk.Frame(button_container, bg=self.bg_color, height=40)
        loop_frame.pack_propagate(False)
        self.loop_cb = ttk.Checkbutton(loop_frame, text="Loop", variable=self.loop_var)
        self.loop_cb.pack(expand=True)
    ...
    def update_ui(self):
        ...
        if self.vlc_player.is_playing():
            ...
            if current_time >= 0 and not self.slider_dragging:
                ...
                # Loop: restart from beginning when reaching end of video (use 3s threshold for reliability)
                elif not self.is_ab_play_mode and self.loop_var.get() and self.video_duration > 3000 and current_time >= self.video_duration - 3000:
                    self.vlc_player.set_time(0)
                    self.slider_var.set(0)
                    self.update_time_display(0)
        else:
            # Loop: when video ended, restart if Loop is checked
            state = self.vlc_player.get_state()
            state_ended = (state == vlc.State.Ended)
            time_near_end = (self.video_duration > 1000 and 
                             self.vlc_player.get_time() >= max(0, self.video_duration - 1000))
            at_end = state_ended or time_near_end
            if (not self.is_ab_play_mode and self.loop_var.get() and self.video_path and at_end):
                self.vlc_player.stop()
