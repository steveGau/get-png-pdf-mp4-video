...
class VideoPlayer:
    ...
    def setup_styles(self):
        ...
        self.style.configure("Tab.TButton", background=self.button_color, foreground=self.fg_color,
                           font=("Segoe UI", 11, "bold"), padding=(16, 8))
        self.style.map("Tab.TButton", background=[("active", self.hover_color)])
        self.style.configure("TabActive.TButton", background=self.accent_color, foreground="#1e1e2e",
                           font=("Segoe UI", 11, "bold"), padding=(16, 8))
        self.style.map("TabActive.TButton", background=[("active", "#b4befe")])
        
    def setup_gui(self):
        """Setup the main GUI: shared video/playback plus Home / Extract Video / Extract PNG tabs."""
        def time_entry(parent, var, width=10, font_size=11):
            return tk.Entry(
                parent, textvariable=var, font=("Consolas", font_size), width=width,
                bg=self.button_color, fg=self.fg_color, insertbackground=self.fg_color,
                relief=tk.FLAT, bd=5,
            )
        ...
        self.auto_extract_stop_var = tk.StringVar(value="01:00:000")
        ...
        self.dumb_video_var = tk.BooleanVar(value=False)
        ...
        # Row 0: tabs (replaces the old title row)
        tab_bar = ttk.Frame(self.main_frame, style="TFrame")
        tab_bar.grid(row=0, column=0, sticky="ew", pady=(0, 4))
        self.tab_buttons = {}
        for key, label in (
            ("home", "Home"),
            ("extract_video", "Extract Video"),
            ("extract_png", "Extract PNG"),
        ):
            btn = ttk.Button(
                tab_bar, text=label, style="Tab.TButton",
                command=lambda k=key: self._show_tab(k),
            )
            btn.pack(side=tk.LEFT, padx=2)
            self.tab_buttons[key] = btn

        self.frame_res_label = tk.Label(
            tab_bar, text="[No video]", font=("Consolas", 9),
            bg=self.bg_color, fg="#f9e2af",
        )
        self.frame_res_label.pack(side=tk.RIGHT, padx=10)

        # Row 1: video display (expands)
        self.video_container = tk.Frame(self.main_frame, bg="#11111b", relief=tk.SUNKEN, bd=2)
        self.video_container.grid(row=1, column=0, sticky="nsew", pady=4)
        self.video_container.grid_propagate(False)
        self.video_container.config(width=900, height=480)
        self.video_label = tk.Label(
            self.video_container, bg="#11111b",
        ...
        # Row 2: seek bar
        time_frame = ttk.Frame(self.main_frame, style="TFrame")
        time_frame.grid(row=2, column=0, sticky="ew", pady=(2, 2))
        self.current_time_label = tk.Label(
            time_frame, text="00:00:00", font=("Consolas", 12, "bold"),
            bg=self.bg_color, fg=self.fg_color,
        )
        ...
        # Row 3: shared playback (same on every tab)
        play_row = ttk.Frame(self.main_frame, style="TFrame")
        play_row.grid(row=3, column=0, sticky="ew", pady=(2, 2))

        self.open_btn = ttk.Button(play_row, text="📂 Open Video",
                                  command=self.open_video, style="Accent.TButton")
        self.open_btn.pack(side=tk.LEFT, padx=4)

        ui_label(play_row, "Step Time:").pack(side=tk.LEFT, padx=(8, 4))
        self.step_time_entry = tk.Entry(
            play_row, textvariable=self.step_time_var, font=("Consolas", 11), width=8,
            bg=self.button_color, fg=self.fg_color, insertbackground=self.fg_color,
            relief=tk.FLAT, bd=5, justify=tk.CENTER,
        )
        self.step_time_entry.pack(side=tk.LEFT, padx=(0, 4))
        ui_label(play_row, "sec").pack(side=tk.LEFT, padx=(0, 8))

        self.fast_bwd_btn = ttk.Button(play_row, text="⏪ Backward",
                                       command=self.fast_backward, style="TButton")
        self.fast_bwd_btn.pack(side=tk.LEFT, padx=3)
        self.play_btn = ttk.Button(play_row, text="▶ Play",
                                  command=self.play_video, style="TButton")
        self.play_btn.pack(side=tk.LEFT, padx=3)
        self.pause_btn = ttk.Button(play_row, text="⏸ Pause",
                                   command=self.pause_video, style="TButton")
        self.pause_btn.pack(side=tk.LEFT, padx=3)
        self.stop_btn = ttk.Button(play_row, text="⏹ Stop",
                                  command=self.stop_video, style="TButton")
        self.stop_btn.pack(side=tk.LEFT, padx=3)
        self.fast_fwd_btn = ttk.Button(play_row, text="⏩ Forward",
                                       command=self.fast_forward, style="TButton")
        self.fast_fwd_btn.pack(side=tk.LEFT, padx=3)

        ui_label(play_row, "Go to time:").pack(side=tk.LEFT, padx=(10, 4))
        self.time_entry = time_entry(play_row, self.time_entry_var, font_size=12)
        self.time_entry.pack(side=tk.LEFT, padx=4)
        self.time_entry.bind("<Return>", self.on_time_entry_change)
        ttk.Button(play_row, text="Go", command=self.on_time_entry_change,
                  style="TButton").pack(side=tk.LEFT, padx=3)

        self.ab_play_btn = ttk.Button(play_row, text="🔁 AB Play Time",
                                      command=self.ab_play_time, style="Extract.TButton")
        self.ab_play_btn.pack(side=tk.LEFT, padx=6)

        # Row 4: tab-specific controls (stacked; one raised at a time)
        self.tab_panels_host = ttk.Frame(self.main_frame, style="TFrame")
        self.tab_panels_host.grid(row=4, column=0, sticky="ew", pady=(2, 2))
        self.tab_panels_host.grid_columnconfigure(0, weight=1)

        self.home_panel = ttk.Frame(self.tab_panels_host, style="TFrame")
        self.extract_video_panel = ttk.Frame(self.tab_panels_host, style="TFrame")
        self.extract_png_panel = ttk.Frame(self.tab_panels_host, style="TFrame")
        self.tab_panels = {
            "home": self.home_panel,
            "extract_video": self.extract_video_panel,
            "extract_png": self.extract_png_panel,
        }
        for panel in self.tab_panels.values():
            panel.grid(row=0, column=0, sticky="ew")

        # --- Home tab ---
        home_row = self.home_panel
        self.start_time_btn = ttk.Button(home_row, text="Start Time",
                                         command=self.on_start_time_click, style="TButton")
        self.start_time_btn.pack(side=tk.LEFT, padx=4)
        self.start_time_entry = time_entry(home_row, self.start_time_var)
        self.start_time_entry.pack(side=tk.LEFT, padx=(0, 8))

        self.end_time_btn = ttk.Button(home_row, text="End Time",
                                       command=self.on_end_time_click, style="TButton")
        self.end_time_btn.pack(side=tk.LEFT, padx=4)
        self.end_time_entry = time_entry(home_row, self.end_time_var)
        self.end_time_entry.pack(side=tk.LEFT, padx=(0, 8))

        self.get_video_time_btn = ttk.Button(home_row, text="⏱ Get Video Time",
                                            command=self.get_video_time, style="TButton")
        self.get_video_time_btn.pack(side=tk.LEFT, padx=6)

        ttk.Button(home_row, text="Set", command=self.on_custom_speed_change,
                  style="TButton").pack(side=tk.LEFT, padx=4)
        for speed in (0.25, 0.5, 1.0, 1.5, 2.0, 4.0):
            ttk.Button(
                home_row, text=f"{speed}x",
                command=lambda s=speed: self.set_speed(s), style="TButton",
            ).pack(side=tk.LEFT, padx=2)

        ui_label(home_row, "Playback Speed:").pack(side=tk.LEFT, padx=(10, 4))
        self.custom_speed_entry = tk.Entry(
            home_row, textvariable=self.custom_speed_var, font=("Consolas", 11), width=6,
            bg=self.button_color, fg=self.fg_color, insertbackground=self.fg_color,
            relief=tk.FLAT, bd=5, justify=tk.CENTER,
        )
        self.custom_speed_entry.pack(side=tk.LEFT, padx=4)
        self.custom_speed_entry.bind("<Return>", self.on_custom_speed_change)
        self.speed_label = tk.Label(
            home_row, text="Speed: 1.0x", font=("Segoe UI", 10),
            bg=self.bg_color, fg=self.accent_color,
        )
        self.speed_label.pack(side=tk.LEFT, padx=8)

        # --- Extract Video tab (same Start/End vars as Home) ---
        ve_row = self.extract_video_panel
        ttk.Button(ve_row, text="Start Time", command=self.on_start_time_click,
                  style="TButton").pack(side=tk.LEFT, padx=4)
        time_entry(ve_row, self.start_time_var).pack(side=tk.LEFT, padx=(0, 8))
        ttk.Button(ve_row, text="End Time", command=self.on_end_time_click,
                  style="TButton").pack(side=tk.LEFT, padx=4)
        time_entry(ve_row, self.end_time_var).pack(side=tk.LEFT, padx=(0, 8))

        ui_label(ve_row, "Rotate Video:").pack(side=tk.LEFT, padx=(6, 4))
        self.rotate_video_combo = ttk.Combobox(
            ve_row, textvariable=self.rotate_video_var,
            values=self.rotate_video_options, state="readonly", width=16,
        )
        self.rotate_video_combo.pack(side=tk.LEFT, padx=4)

        self.extract_video_btn = ttk.Button(ve_row, text="🎬 Extract Video",
                                            command=self.extract_video, style="Extract.TButton")
        self.extract_video_btn.pack(side=tk.LEFT, padx=6)
        ttk.Checkbutton(ve_row, text="Dumb Video", variable=self.dumb_video_var).pack(
            side=tk.LEFT, padx=6
        )
        self.clear_mp4_btn = ttk.Button(ve_row, text="🗑 Clear All MP4s",
                                        command=self.clear_all_mp4s, style="Danger.TButton")
        self.clear_mp4_btn.pack(side=tk.LEFT, padx=6)

        # --- Extract PNG tab ---
        png_row = self.extract_png_panel
        ui_label(png_row, "Auto extract start:").pack(side=tk.LEFT, padx=(4, 4))
        self.auto_extract_start_entry = time_entry(png_row, self.auto_extract_start_var, font_size=12)
        self.auto_extract_start_entry.pack(side=tk.LEFT, padx=4)
        ttk.Button(png_row, text="Copy Go", command=self.copy_go_to_auto_extract_start,
                  style="TButton").pack(side=tk.LEFT, padx=2)

        ui_label(png_row, "Auto extract stop:").pack(side=tk.LEFT, padx=(8, 4))
        self.auto_extract_stop_entry = time_entry(png_row, self.auto_extract_stop_var, font_size=12)
        self.auto_extract_stop_entry.pack(side=tk.LEFT, padx=4)
        ttk.Button(png_row, text="Copy Time", command=self.copy_start_end_to_auto_extract,
                  style="TButton").pack(side=tk.LEFT, padx=2)

        ui_label(png_row, "Every:").pack(side=tk.LEFT, padx=(8, 4))
        self.interval_entry = time_entry(png_row, self.interval_entry_var, font_size=12)
        self.interval_entry.pack(side=tk.LEFT, padx=4)

        self.get_video_time_btn_png = ttk.Button(
            png_row, text="⏱ Get Video Time",
            command=self.get_video_time, style="TButton",
        )
        self.get_video_time_btn_png.pack(side=tk.LEFT, padx=6)

        self.extract_btn = ttk.Button(png_row, text="📸 Extract Frame (PNG)",
                                     command=self.extract_frame, style="Extract.TButton")
        self.extract_btn.pack(side=tk.LEFT, padx=6)
        self.auto_extract_btn = ttk.Button(png_row, text="🎞️ Auto Extract Frame (PNG)",
                                          command=self.auto_extract_frames, style="Extract.TButton")
        self.auto_extract_btn.pack(side=tk.LEFT, padx=6)
        self.create_pdf_btn = ttk.Button(png_row, text="📄 Create PDF",
                                        command=self.create_pdf, style="Extract.TButton")
        self.create_pdf_btn.pack(side=tk.LEFT, padx=6)
        self.clear_btn = ttk.Button(png_row, text="🗑 Clear All PNGs",
        ...
        self.status_bar.grid(row=5, column=0, sticky="ew", pady=(4, 0))
    ...
    def _show_tab(self, key):
        """Show one tab panel. Video, seek bar, and playback row stay in place."""
        panel = self.tab_panels.get(key)
        if panel is None:
            return
        panel.tkraise()
        for name, btn in self.tab_buttons.items():
            btn.configure(style="TabActive.TButton" if name == key else "Tab.TButton")

    def on_resize(self, event=None):
        """Handle window resize to adjust video container."""
        if event and event.widget == self.root:
            total_height = self.root.winfo_height()
            # Tabs + seek + 2 control rows + status (video gets the rest)
            video_height = max(360, total_height - 250)
    ...
    def copy_start_end_to_auto_extract(self):
        """Copy Start Time / End Time into Auto extract start / Auto extract stop."""
        start_str = self.start_time_var.get().strip()
        end_str = self.end_time_var.get().strip()
        self.auto_extract_start_var.set(start_str)
        self.auto_extract_stop_var.set(end_str)
        self.status_var.set(
            f"Auto extract start/stop set to {start_str} / {end_str}"
        )
    ...
    def _ffmpeg_trim_input(self, start_ffmpeg, duration_ffmpeg, accurate_decode):
        """
        Fast seek (-ss before -i) for 1.0x re-encode (still frame-accurate).
        Output-side seek (-ss after -i) when filters must decode every frame
        (fixes Invalid NAL unit errors on already-trimmed segment files).
        """
        if accurate_decode:
    ...
    def _optional_vf_args(self, vf):
        """Return `-filter:v` args when a filter chain is needed; otherwise empty."""
        return ["-filter:v", vf] if vf else []
    ...
    def _extract_av_reencode(self, trim_args, vf, output_av, temp_dir, start_ffmpeg, duration_ffmpeg):
        """Re-encode video (optionally filtered) with audio; fallbacks for difficult AAC streams."""
        tail = self._encode_tail_args()
        map_args = ["-map", "0:v:0", "-map", "0:a:0?"] + self._optional_vf_args(vf)
        ok, err = False, None

        # Audio copy + input-side -ss starts audio at the previous keyframe, so
        # the container duration overruns and A/V drift. Only copy audio when a
        # filter forced output-side -ss (decode to the exact timestamp first).
        if vf:
            combined_copy_a = (
                self._ffmpeg_base()
                + trim_args
        ...
        # Re-encode both video and audio in one pass (frame-accurate 1.0x path)
        combined = (
            self._ffmpeg_base()
            + trim_args
            + map_args
    ...
    def extract_video(self):
        ...
        dumb_video = self.dumb_video_var.get()
        output_dumb = os.path.join(mp4_folder, f"{base_name}_dumb.mp4")
        ...
        try:
            ...
            if dumb_video:
                self.status_var.set(f"Extracting video at {speed}x (dumb video, no audio)...")
                self.root.update()
                ...
                dumb_cmd = (
                    ...
                    + ["-an"]
                    + self._reencode_video_codec_args()
                    + ["-max_muxing_queue_size", "1024", "-avoid_negative_ts", "make_zero", output_dumb]
                )
                ok_dumb, err_dumb = self._run_ffmpeg(dumb_cmd, "Dumb video (no audio)")
    ...
    def _resolve_auto_extract_window(self, start_str, stop_str, interval_str):
        """Resolve Auto extract start / stop / Every into millisecond bounds.

        Stop is an absolute timestamp (not a duration). End is capped at video length.

        Returns a dict with ok, start_ms, end_ms, interval_ms, capped,
        error_title, and error_msg.
        """
        result = {
            "ok": False,
            "start_ms": None,
        ...
        start_ms = self.time_hires_to_ms(start_str)
        if start_ms is None or start_ms < 0:
            result["error_title"] = "Invalid Start Time"
            result["error_msg"] = (
                "Please enter a valid auto extract start in format MM:SS:mmm (e.g., 00:00:000)"
            )
            return result

        stop_ms = self.time_hires_to_ms(stop_str)
        if stop_ms is None or stop_ms < 0:
            result["error_title"] = "Invalid Stop Time"
            result["error_msg"] = (
                "Please enter a valid auto extract stop in format MM:SS:mmm (e.g., 01:00:000)"
            )
            return result

        interval_ms = self.interval_to_ms(interval_str)
        if interval_ms is None or interval_ms <= 0:
            result["error_title"] = "Invalid Interval"
            result["error_msg"] = (
                "Please enter a valid interval in format MM:SS:mmm (must be > 0)"
            )
            return result
        ...
        if stop_ms <= start_ms:
            result["error_title"] = "Invalid Range"
            result["error_msg"] = "Auto extract stop must be after auto extract start."
            return result

        end_ms = min(stop_ms, self.video_duration)
        if end_ms - start_ms <= 0:
            result["error_title"] = "Invalid Range"
            result["error_msg"] = "No frames to extract in the specified range."
            return result
        ...
        result["stop_ms"] = stop_ms
        result["capped"] = stop_ms > self.video_duration
        return result

    def auto_extract_frames(self):
        """Automatically extract frames from Auto extract start to Auto extract stop, every interval."""
        if self.cap is None or self.video_path is None:
            messagebox.showwarning("Warning", "Please open a video file first.")
            return

        # Bounds come from Auto extract start / stop (not Go to time — Go tracks playback)
        interval_str = self.interval_entry_var.get().strip()
        window = self._resolve_auto_extract_window(
            self.auto_extract_start_var.get().strip(),
            self.auto_extract_stop_var.get().strip(),
            interval_str,
        )
        if not window["ok"]:
            messagebox.showwarning(window["error_title"], window["error_msg"])
            return
        ...
        msg = (
            ...
            f"Stop Time: {self.ms_to_time_hires(end_ms)}\n"
            f"Duration: {self.ms_to_time_hires(actual_duration_ms)}"
        )
        if window["capped"]:
            msg += (
                f"\n(requested stop {self.ms_to_time_hires(window['stop_ms'])}, "
