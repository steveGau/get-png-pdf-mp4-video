...
class VideoPlayer:
    ...
    def setup_gui(self):
        ...
        # --- Time / auto-extract settings (two rows) ---
        time_input_frame = ttk.Frame(controls_container, style="TFrame")
        time_input_frame.pack(fill=tk.X, pady=(0, 4))

        time_row_a = ttk.Frame(time_input_frame, style="TFrame")
        time_row_a.pack(fill=tk.X, pady=2)
        time_row_b = ttk.Frame(time_input_frame, style="TFrame")
        time_row_b.pack(fill=tk.X, pady=2)

        tk.Label(time_row_a, text="Go to time:", font=("Segoe UI", 10),
                bg=self.bg_color, fg=self.fg_color).pack(side=tk.LEFT, padx=(10, 5))
        self.time_entry_var = tk.StringVar(value="00:00:000")
        self.time_entry = tk.Entry(time_row_a, textvariable=self.time_entry_var,
        ...
        ttk.Button(time_row_a, text="Go", command=self.on_time_entry_change,
                  style="TButton").pack(side=tk.LEFT, padx=5)

        tk.Label(time_row_a, text="|", font=("Segoe UI", 10),
                bg=self.bg_color, fg="#585b70").pack(side=tk.LEFT, padx=8)

        tk.Label(time_row_a, text="Auto extract start:", font=("Segoe UI", 10),
        ...
        self.auto_extract_start_entry = tk.Entry(
            time_row_a, textvariable=self.auto_extract_start_var,
            font=("Consolas", 12), width=10, bg=self.button_color,
            fg=self.fg_color, insertbackground=self.fg_color,
            relief=tk.FLAT, bd=5,
        )
        self.auto_extract_start_entry.pack(side=tk.LEFT, padx=5)
        ttk.Button(time_row_a, text="Copy Go",
                  command=self.copy_go_to_auto_extract_start, style="TButton").pack(side=tk.LEFT, padx=2)

        tk.Label(time_row_a, text="Auto extract stop:", font=("Segoe UI", 10),
                bg=self.bg_color, fg=self.fg_color).pack(side=tk.LEFT, padx=(10, 5))
        self.auto_extract_stop_var = tk.StringVar(value="01:00:000")
        self.auto_extract_stop_entry = tk.Entry(
            time_row_a, textvariable=self.auto_extract_stop_var,
            font=("Consolas", 12), width=10, bg=self.button_color,
            fg=self.fg_color, insertbackground=self.fg_color,
            relief=tk.FLAT, bd=5,
        )
        self.auto_extract_stop_entry.pack(side=tk.LEFT, padx=5)
        ttk.Button(time_row_a, text="Copy Time",
                  command=self.copy_start_end_to_auto_extract, style="TButton").pack(side=tk.LEFT, padx=2)

        tk.Label(time_row_b, text="Every:", font=("Segoe UI", 10),
                bg=self.bg_color, fg=self.fg_color).pack(side=tk.LEFT, padx=(10, 5))
        self.interval_entry_var = tk.StringVar(value="00:00:100")
        self.interval_entry = tk.Entry(time_row_b, textvariable=self.interval_entry_var,
                                      font=("Consolas", 12), width=10, bg=self.button_color,
                                      fg=self.fg_color, insertbackground=self.fg_color,
                                      relief=tk.FLAT, bd=5)
        self.interval_entry.pack(side=tk.LEFT, padx=5)

        self.frame_res_label = tk.Label(time_row_b, text="[No video]",
                                        font=("Consolas", 9), bg=self.bg_color, fg="#f9e2af")
        self.frame_res_label.pack(side=tk.LEFT, padx=(15, 5))

        self.speed_label = tk.Label(time_row_b, text="Speed: 1.0x", font=("Segoe UI", 10),
        ...
        # --- Video extract / MP4 (two rows) ---
        extract_frame = ttk.Frame(controls_container, style="TFrame")
        extract_frame.pack(fill=tk.X, pady=4)

        video_extract_row = ttk.Frame(extract_frame, style="TFrame")
        video_extract_row.pack(fill=tk.X, pady=(0, 6))
        video_utils_row = ttk.Frame(extract_frame, style="TFrame")
        video_utils_row.pack(fill=tk.X, pady=(0, 6))
        png_row = ttk.Frame(extract_frame, style="TFrame")
        png_row.pack(fill=tk.X)

        # Video extract row
        ve_inner = ttk.Frame(video_extract_row, style="TFrame")
        ve_inner.pack(anchor=tk.CENTER)

        tk.Label(ve_inner, text="Rotate Video:", font=("Segoe UI", 10),
                bg=self.bg_color, fg=self.fg_color).pack(side=tk.LEFT, padx=(6, 4))
        ...
        self.extract_video_btn = ttk.Button(ve_inner, text="🎬 Extract Video",
        ...
        ttk.Checkbutton(
            ve_inner,
            text="Dumb Video",
            variable=self.dumb_video_var,
        ).pack(side=tk.LEFT, padx=6)

        self.start_time_btn = ttk.Button(ve_inner, text="Start Time",
        ...
        self.start_time_entry = tk.Entry(ve_inner, textvariable=self.start_time_var,
                                         font=("Consolas", 11), width=10, bg=self.button_color,
                                         fg=self.fg_color, insertbackground=self.fg_color,
                                         relief=tk.FLAT, bd=5)
        self.start_time_entry.pack(side=tk.LEFT, padx=(0, 4))

        self.end_time_btn = ttk.Button(ve_inner, text="End Time",
        ...
        self.end_time_entry = tk.Entry(ve_inner, textvariable=self.end_time_var,
                                       font=("Consolas", 11), width=10, bg=self.button_color,
                                       fg=self.fg_color, insertbackground=self.fg_color,
                                       relief=tk.FLAT, bd=5)
        self.end_time_entry.pack(side=tk.LEFT, padx=(0, 4))

        # Video utilities row
        vu_inner = ttk.Frame(video_utils_row, style="TFrame")
        vu_inner.pack(anchor=tk.CENTER)

        self.ab_play_btn = ttk.Button(vu_inner, text="🔁 AB Play Time",
                                      command=self.ab_play_time, style="Extract.TButton")
        self.ab_play_btn.pack(side=tk.LEFT, padx=6)

        self.clear_mp4_btn = ttk.Button(vu_inner, text="🗑 Clear All MP4s",
                                        command=self.clear_all_mp4s, style="Danger.TButton")
        self.clear_mp4_btn.pack(side=tk.LEFT, padx=6)

        self.get_video_time_btn = ttk.Button(vu_inner, text="⏱ Get Video Time",
                                            command=self.get_video_time, style="TButton")
        self.get_video_time_btn.pack(side=tk.LEFT, padx=6)

        # PNG / PDF row
        png_inner = ttk.Frame(png_row, style="TFrame")
        png_inner.pack(anchor=tk.CENTER)

        self.extract_btn = ttk.Button(png_inner, text="📸 Extract Frame (PNG)",
                                     command=self.extract_frame, style="Extract.TButton")
        self.extract_btn.pack(side=tk.LEFT, padx=6)

        self.auto_extract_btn = ttk.Button(png_inner, text="🎞️ Auto Extract Frame (PNG)",
                                          command=self.auto_extract_frames, style="Extract.TButton")
        self.auto_extract_btn.pack(side=tk.LEFT, padx=6)

        self.create_pdf_btn = ttk.Button(png_inner, text="📄 Create PDF",
                                        command=self.create_pdf, style="Extract.TButton")
        self.create_pdf_btn.pack(side=tk.LEFT, padx=6)

        self.clear_btn = ttk.Button(png_inner, text="🗑 Clear All PNGs",
