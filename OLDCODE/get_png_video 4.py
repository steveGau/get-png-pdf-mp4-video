...
class VideoPlayer:
    ...
    def calculate_lowest_speed(self):
        ...
        The lowest speed is calculated as 1/fps, which means each frame 
        is displayed for approximately 1 second at this speed.
        """
        if self.fps > 0:
            # Lowest speed = 1/fps (each frame displayed for ~1 second)
            lowest_speed = 1.0 / self.fps
            # Round to 4 decimal places for readability
            return round(lowest_speed, 4)
        return 0.0333  # Default if fps is unknown (assuming ~30fps)
